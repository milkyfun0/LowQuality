...
def parse_option():
    ...
    parser.add_argument('--learning_rate', type=float, default=1e-4)  # 0.05
    ...
    parser.add_argument('--num_embedding', default=5000, type=int)
    parser.add_argument('--topk', default=10, type=int)
    parser.add_argument('--data_type', default="vq")
    parser.add_argument('--cor_name', default="motion_blur", type=str)  # gaussian_noise  gaussian_blur fog motion_blur
    parser.add_argument('--method',
                        default="alexnet_wtopk10_twovq(5000afterhash)_fc_1.0pairwise_motion_blur(s3)_AID_adam",
                        type=str)  # alexnet_pairwise(margin1.0)_ucmd
    parser.add_argument('--dataset', type=str, default='AID')
    parser.add_argument('--root_path', default='../data/AID/original/')
    parser.add_argument('--txtfile_path', default='txtfile/')
    parser.add_argument('--save_path', default='/home/huangmengluan/retrieval/low-quality-2024-2-4/log/AID/')
...
def train(train_loader, test_loader, dataset_loader, bit, args):
    net = AlexNet(hash_bit=bit, classes=30, num_embedding=args.num_embedding, topk=args.topk).cuda()
    ...
    for epoch in range(args.epochs):
        ...
        for idx, (image, distort_image, label, ind) in enumerate(tqdm(train_loader, desc=f"train {epoch}")):
            ...
            loss = loss_pair + loss_cls + e_q_loss_clean + e_q_loss_distort
        ...
        logger.info(
            '[epoch:{}/{}][pair_loss:{:.6f}][cls_loss:{:.6f}][vq_loss:{:.6f}][total_loss:{:.6f}]'.format(
